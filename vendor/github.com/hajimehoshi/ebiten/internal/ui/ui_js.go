...
func setMouseCursorFromEvent(e *js.Object) {
	//scale := canvas.Get("dataset").Get("ebitenScale").Float()
	rect := canvas.Call("getBoundingClientRect")
	scale := (rect.Get("right").Float() - rect.Get("left").Float()) / canvas.Get("dataset").Get("ebitenWidth").Float()
	x, y := e.Get("clientX").Float(), e.Get("clientY").Float()
	x -= rect.Get("left").Float()
	y -= rect.Get("top").Float()
	currentInput.SetMouseCursor(int(x/scale), int(y/scale))
}
...
func (u *userInterface) setScreenSize(width, height, scale int) bool {
	...
	canvas.Get("dataset").Set("ebitenWidth", width)
